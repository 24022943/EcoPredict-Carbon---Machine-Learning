...
- `scenario_projection.py`: mô phỏng kịch bản tham số.
...
- `generate_shap_explanations.py`: sinh SHAP plots từ model package.
...
### 4.8 Mô phỏng kịch bản

Hệ thống không sử dụng ARIMA vì dữ liệu PCF không phải chuỗi thời gian liên tục.

Thay vào đó, EcoPredict Carbon sử dụng mô phỏng kịch bản theo các yếu tố vòng đời như vật liệu, năng lượng, vận chuyển, sử dụng và xử lý cuối vòng đời.

### 4.9 Công nghệ sử dụng

Hệ thống được xây dựng bằng Python và Streamlit.

Các thư viện chính gồm Pandas, NumPy, Scikit-learn, Imbalanced-learn, SHAP, Plotly, Matplotlib và Joblib.
