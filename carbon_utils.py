...
def enrich_with_reference_factors(df: pd.DataFrame, openpcf: pd.DataFrame, ceda: pd.DataFrame) -> pd.DataFrame:
    """Bổ sung hệ số OpenPCF/Open CEDA một cách an toàn.

    Fix lỗi Streamlit Cloud:
    - Không dùng out.get(..., np.nan).fillna(...), vì np.nan là float và không có fillna().
    - Nếu cột chưa tồn tại thì tạo Series mặc định theo index của DataFrame.
    - Ưu tiên map theo industry_group, country rồi dùng median toàn cục.
    """
    out = df.copy()

    def ensure_series(col: str, default: float = np.nan) -> pd.Series:
        if col not in out.columns:
            return pd.Series(default, index=out.index, dtype="float64")
        return pd.to_numeric(out[col], errors="coerce")

    def safe_median(frame: pd.DataFrame, col: str, default: float) -> float:
    ...
    # OpenPCF factor kgCO2e/kg: product/material reference.
    out["openpcf_factor_kgco2e_per_kg"] = ensure_series("openpcf_factor_kgco2e_per_kg")
    openpcf_default = safe_median(openpcf, "openpcf_factor_kgco2e_per_kg", 1.0)
    if openpcf is not None and not openpcf.empty and "openpcf_factor_kgco2e_per_kg" in openpcf.columns:
        open_ref = openpcf.copy()
        open_ref["openpcf_factor_kgco2e_per_kg"] = pd.to_numeric(open_ref["openpcf_factor_kgco2e_per_kg"], errors="coerce")
        open_ref = open_ref.dropna(subset=["openpcf_factor_kgco2e_per_kg"])
        if "industry_group" in open_ref.columns and "industry_group" in out.columns:
            industry_mat = open_ref.groupby("industry_group")["openpcf_factor_kgco2e_per_kg"].median().to_dict()
        ...
        if "country" in open_ref.columns and "country" in out.columns:
            country_mat = open_ref.groupby("country")["openpcf_factor_kgco2e_per_kg"].median().to_dict()
            missing = out["openpcf_factor_kgco2e_per_kg"].isna()
            out.loc[missing, "openpcf_factor_kgco2e_per_kg"] = out.loc[missing, "country"].map(country_mat)
    out["openpcf_factor_kgco2e_per_kg"] = pd.to_numeric(out["openpcf_factor_kgco2e_per_kg"], errors="coerce").fillna(openpcf_default).clip(lower=0.000001)

    # Open CEDA spend-based/context factor kgCO2e/USD: support/context only.
    out["ceda_factor_kgco2e_per_usd"] = ensure_series("ceda_factor_kgco2e_per_usd")
    ceda_default = safe_median(ceda, "ceda_factor_kgco2e_per_usd", 0.5)
    if ceda is not None and not ceda.empty and "ceda_factor_kgco2e_per_usd" in ceda.columns and "country" in ceda.columns and "country" in out.columns:
        lookup = country_factor_lookup(ceda)
        missing = out["ceda_factor_kgco2e_per_usd"].isna()
        out.loc[missing, "ceda_factor_kgco2e_per_usd"] = out.loc[missing, "country"].map(lookup)
    out["ceda_factor_kgco2e_per_usd"] = pd.to_numeric(out["ceda_factor_kgco2e_per_usd"], errors="coerce").fillna(ceda_default).clip(lower=0.0)
...
def openpcf_stratified_split(
    df: pd.DataFrame,
    target_col: str = TARGET_COL,
    test_size: float = 0.20,
    random_state: int = RANDOM_STATE,
) -> tuple[pd.DataFrame, pd.DataFrame, str]:
    """Chia train/test ưu tiên OpenPCF đầy đủ và bảo đảm test có đủ Low/Medium/High.

    Lý do: OpenPCF chủ yếu cùng năm 2025 nên time split sẽ dễ tạo test lệch lớp.
    Hàm này tạo nhãn tạm theo q25/q75 trên toàn tập chỉ để stratify split, sau đó
    train_advanced_models.py sẽ fit lại ngưỡng nhãn trên train-only để tránh leakage
    trong đánh giá chính thức.
    """
    d = df.dropna(subset=[target_col]).copy().reset_index(drop=True)
    d = d[d[target_col] > 0].reset_index(drop=True)
    if len(d) < 10:
        train, test, marker = time_based_split(d)
        return train, test, f"fallback_time_split_{marker}"

    tmp_thresholds = fit_label_thresholds(d, target_col=target_col)
    stratify_labels = apply_carbon_labels(d, tmp_thresholds, target_col=target_col)
    ...
    train = d.iloc[train_idx].copy().reset_index(drop=True)
    test = d.iloc[test_idx].copy().reset_index(drop=True)
    return train, test, "openpcf_stratified_80_20"
...
def get_classification_models() -> dict[str, Any]:
    ...
    Bản OpenPCF-focused ưu tiên mô hình cây ensemble có class_weight để cải thiện lớp High.
    Balanced Random Forest được thêm nếu imbalanced-learn có sẵn; nếu không thì fallback an toàn.
    """
    models: dict[str, Any] = {
...
def get_regression_models() -> dict[str, Any]:
    ...
    Target được log-transform để xử lý phân phối PCF lệch phải và các giá trị cực trị.
    """
    return {
...
def make_clf_pipeline(model: Any, use_smote: bool = False, smote_k_neighbors: int = 3, sampler: str = "smotetomek") -> Pipeline:
    """Tạo pipeline phân loại.

    Nếu use_smote=True và đã cài imbalanced-learn, pipeline sẽ là:
    preprocessor -> SMOTE/SMOTETomek -> model.

    Lưu ý: sampler chỉ chạy trong fit() trên train. Test set luôn giữ dữ liệu thật.
    """
    ...
    if use_smote:
        try:
            ...
            if sampler.lower() in {"smotetomek", "smote_tomek"}:
                sampler_step = SMOTETomek(smote=smote, random_state=RANDOM_STATE)
            else:
                sampler_step = smote
            return ImbPipeline([
